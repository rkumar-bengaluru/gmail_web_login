  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      ...
      {permissionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-md">
            <h2 className="text-lg font-semibold mb-4">Allow Microphone and Camera Access</h2>
            <button onClick={handlePermission} className="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">
              Continue
            </button>
          </div>
        </div>
      ...
      {scopeModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-md">
            <h2 className="text-lg font-semibold mb-4">Allow access for this session or always?</h2>
            <button onClick={handleScope} className="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">
        ...
        <div className="flex w-full max-w-5xl bg-white shadow-lg rounded-lg overflow-hidden">
          ...
          <div className="w-2/3 bg-black relative">
            ...
            <div className="absolute bottom-4 left-4 flex gap-4">
              <button onClick={toggleAudio} className="bg-white p-2 rounded-full shadow">
                {audioEnabled ? (
                  <span className={`text-green-600 ${isSpeaking ? 'animate-pulse' : ''}`}>🎤</span>
                ) : (
                  <span className="text-red-600">🔇</span>
                )}
              </button>
              <button onClick={toggleVideo} className="bg-white p-2 rounded-full shadow">
                {videoEnabled ? <span className="text-green-600">📹</span> : <span className="text-red-600">📷</span>}
              </button>
            </div>
          </div>
